...
export async function POST(req: NextRequest) {
  ...
  // Look up any order info for this customer
  ...
  if (result.confident) {
    ...
    if (result.action === "send_reset_link") {
      // Trigger a password reset email if they have an account
      try {
        await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/auth/forgot-password`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email, captchaToken: "bypass_internal" }),
        });
      } catch {}
    }

    if (result.action === "resend_tickets") {
      // Trigger ticket resend
      try {
        await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/admin/resend-tickets`, {
          method: "POST",
    ...
    return NextResponse.json({ handled: true, action: result.action });
...
function buildReplyHtml(customerName: string, replyText: string): string {
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0d0500;padding:40px 20px">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
        ...
        <tr><td style="background:rgba(245,166,35,0.08);border:2px solid rgba(245,166,35,0.4);border-radius:16px;padding:24px;margin-bottom:16px">
