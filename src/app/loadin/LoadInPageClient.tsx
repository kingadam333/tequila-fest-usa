          ...
          <>
            {!selected ? (
              <motion.div key="selector" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                ...
                {events.length === 0 ? (
                  <p className="text-white/80 text-center">No upcoming events found.</p>
                ) : (
                    ...
                    {events.map(ev => {
                      ...
                      return (
                        <button key={ev.id} onClick={() => setSelected(ev)}
                          ...
                          <p className="text-white/80 text-sm flex items-center gap-1.5"><Calendar size={13} /> {ev.date}</p>
                          {ev.venue && <p className="text-white/80 text-xs mt-1 flex items-center gap-1.5"><MapPin size={12} /> {ev.venue}</p>}
                        </button>
                      );
                    })}
                  </div>
              ...
              <motion.div key="detail" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <button onClick={() => setSelected(null)}
                  className="flex items-center gap-1.5 text-white/80 hover:text-white text-sm mb-6 transition-colors cursor-pointer">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 mb-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-white/80 text-xs uppercase tracking-wider">Date & Time</p>
                        <p className="text-white font-semibold">{selected.date}</p>
                        {selected.time && <p className="text-white/80 text-sm">{selected.time}</p>}
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        {selected.venueDetail && <p className="text-white/80 text-sm">{selected.venueDetail}</p>}
                ...
                {(selected.loadInStart || selected.loadInEnd) && (
                  <div className="rounded-2xl border p-6 mb-5" style={{ borderColor: `${cityColor(selected.city)}40`, background: `${cityColor(selected.city)}0d` }}>
                    ...
                    <p className="text-white/80 text-lg mt-2">
                      {selected.loadInStart || "TBD"} <span className="text-white/80 mx-1">→</span> {selected.loadInEnd || "TBD"}
                    </p>
                    {selected.loadInEnd && (
                      <p className="text-white/80 text-sm mt-1">You must be on-site and unloaded by {selected.loadInEnd}. Setup after.</p>
                    )}
                  </div>
                )}
                ...
                {selected.loadInNotes && (
                  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 mb-5">
                    <p className="text-white/80 text-xs uppercase tracking-wider mb-2">Details</p>
                ...
                {selected.mapUrl && (
                  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 mb-5">
                    <p className="text-white/80 text-xs uppercase tracking-wider mb-3 px-2">Venue Map</p>
                ...
                {selected.mapUrl2 && (
                  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                    <p className="text-white/80 text-xs uppercase tracking-wider mb-3 px-2">Additional Map</p>
                ...
                {!selected.loadInStart && !selected.loadInNotes && !selected.mapUrl && (
                  <p className="text-white/80 text-sm text-center py-6">Load-in details for this event haven&apos;t been posted yet — check back soon.</p>
                )}
              </motion.div>
            )}
          </>
