...
function ResetPasswordForm() {
  ...
  if (validating) return <div className="text-center text-white/80 py-10">Validating link...</div>;

  if (!token || !tokenValid) return (
    <div className="text-center py-4">
      <p className="text-4xl mb-4">⚠️</p>
      <p className="font-display text-white text-2xl mb-2">INVALID LINK</p>
      <p className="text-white/80 text-sm mb-6">This reset link has expired or is invalid.</p>
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          ...
          <button type="button" onClick={() => setShowPw(v => !v)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/80 hover:text-white/60 cursor-pointer">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md">
          <div className="rounded-3xl border border-white/10 bg-black/40 backdrop-blur-sm p-8 md:p-10">
            <div className="text-center mb-8">
              ...
              <p className="text-white/80 text-sm mt-2">Choose a new password for your account</p>
            </div>
            <Suspense fallback={<div className="text-center text-white/80 py-10">Loading...</div>}>
              <ResetPasswordForm />
            </Suspense>
          </div>
        </motion.div>
